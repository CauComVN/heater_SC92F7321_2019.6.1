...
//	static uint Lower_Limit= 0; //PIDÊä³öÏÂÏŞ
	...
	//Ò»¶¨ÒªÈ¡¸ºµÄ£¬ÒòÎª¹¦ÂÊµ÷½ÚÊÇÏà·´µÄ£¬scr_curr_timeÔ½Ğ¡£¬¹¦ÂÊÔ½´ó
	//Out = -Out;
...
//	}
	...
	/*
					...
					//0 --- 20000  42¶È-28¶È=14¶È 20000/14=1428.57
									
					//Éè¶¨Öµ´óÓÚÊµ¼ÊÖµ·ñ£¿
					//Æ«²î´óÓÚ2ÎªÉÏÏŞ·ùÖµÊä³ö(È«ËÙ¼ÓÈÈ)
					if(best_temp_out-current_out_temp>2) ////Æ«²î´óÓÚ2?
					{
						scr_open_time=0;//8600;//17200;//20000;//5;//µÍµçÆ½ 8.6ms 17200---0  ¸ßµçÆ½ 10ms  20000---0
						scr_curr_time=0;
					}
						...
						//pidret=PIDCalc(best_temp_out,current_out_temp);//0¿ÉÒÔ
						
						pidret=14*pidret;//È¡Ê®·ÖÖ®Ò»À´Ëã£¬²»È»Êı¾İÌ«´ó£¬Òç³öÁË
...
//	if(pidtimer < pidt)     //¼ÆËãÖÜÆÚ   pidtimer¿ÉÒÔÓÃ¶¨Ê±Æ÷¼ÆÊ±
//		return ;  //
...
//	Pout = Kp * DERR1;    //Êä³öP
//	Iout = (float)(ERR * ((Kp * pidt) / Ti));  //Êä³öI
//	Dout = (float)(DERR2 * ((Kp * Td) / pidt));   //Êä³öD

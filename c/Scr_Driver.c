...
//¿¹±¥ºÍ»ı·Ö
//8Î»µ¥Æ¬»ú²»Ö§³Ö¸¡µãÔËËã£¬Ìá¸ß¿ØÖÆ¾«¶È£¬³Ë10´¦Àí
int idata umax=500;//=50;//ÀíÏë×î´óÎÂ¶ÈÖµ
int idata umin=290;//=29;//ÀíÏë×îĞ¡ÎÂ¶ÈÖµ

int idata Out1=0;  		//¼ÇÂ¼ÉÏ´ÎÊä³ö
int idata ERR=0;       //µ±Ç°Îó²î
int idata ERR1=0;      //ÉÏ´ÎÎó²î
int idata ERR2=0;      //ÉÏÉÏ´ÎÎó²î
int idata integral=0;		//»ı·Ö·ÖÀë
...
void PIDCalc(int Sv,int Pv)
{
    ...
    int idata Pout = 0;       //±ÈÀı½á¹û
    int idata Iout = 0;       //»ı·Ö½á¹û
    int idata Dout = 0;       //Î¢·Ö½á¹û
    int idata Out = 0; 				//×ÜÊä³ö
    ...
    //Ki »ı·Ö·ÖÀë¹ı³Ì ¿¹»ı·Ö±¥ºÍ
    if(Pv>umax) { //»ÒÉ«µ×É«±íÊ¾¿¹»ı·Ö±¥ºÍµÄÊµÏÖ

        if(abs(ERR)>1) {    //À¶É«±ê×¢Îª»ı·Ö·ÖÀë¹ı³Ì
            index=0;
        } else {
            index=1;
            if(ERR<0) {
